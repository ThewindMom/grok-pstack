...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake, and before a human report. Arm the frontier watcher with Grok `monitor` on `pstack-watch-pr` or `"$PSTACK_SCRIPTS/watch-pr/watch-pr"`, or a background child that returns on the event, with `pstack-heartbeat 1800` as the fallback. There is no Cursor `/loop`. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts; a dedicated verifier agent (on a different model or effort than the worker) is for units whose verification is expensive, judgment-laden, or high-blast-radius. A verifier agent whose entire product would be rerunning one command is ceremony, not verification.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the Grok tasks pane. Transcript mtime is not liveness.
...
- After a Grok session restart: in-flight children are dead. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach work by PR and branch rather than agent id, spawn fresh workers from stored briefs plus current state, drain, resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
