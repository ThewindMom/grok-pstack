...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake, and before a human report. Arm the frontier watcher with Grok `monitor` on `pstack-watch-pr` or `"$PSTACK_SCRIPTS/watch-pr/watch-pr"`, or a background child that returns on the event, with a long heartbeat fallback. There is no Cursor `/loop`. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
