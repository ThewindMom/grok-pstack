...
1. Capture a baseline trace on the matching surface. Drive the real UI, CLI, API, or profiler yourself. If the project has a `verify-*` skill, use it. If it has no scripted drive path, run `/create-verification-skill` first, then use the generated skill. There is no `control-ui` or `control-cli`.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured perf-issue model (default `grok-4.6` `xhigh`); review the diff. Capture a post-fix trace.
