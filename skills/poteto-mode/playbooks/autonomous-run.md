...
2. Pick the wake mechanism. An event to watch (CI, a merge, a ref advancing) gets Grok `monitor` on the watcher command, or a background child that returns on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking. Do not call Cursor `/loop`. It is not here.
