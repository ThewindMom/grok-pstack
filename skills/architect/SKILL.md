...
Use your configured architect runners from `panels.architect_runners` (defaults: `grok-pstack:pstack-xhigh` adversarial, `grok-pstack:pstack-high` quality, `grok-pstack:pstack-high` mechanical, `grok-pstack:pstack-xhigh` architecture). The parent running `/architect` owns the arena fan-out. Do not expect a child to spawn the runners.
