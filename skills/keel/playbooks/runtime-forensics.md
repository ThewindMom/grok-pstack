...
1. Capture the live signal on the matching surface: a CPU profile for a spinning process, a heap snapshot for a leak, a CDP trace for a visual glitch. Drive the real process yourself. If the project has a `verify-*` skill, use it. If it has no scripted drive path, run `/create-verification-skill` first, then use the generated skill. There is no `control-ui` or `control-cli`. A real artifact, not a guess.
