...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <transcript path>", "you're taking over", "pick up where X left off", a Grok session handoff, or a pushed branch you're meant to continue.
...
1. Locate the prior trail. A local transcript the Grok session names (`~/.grok/sessions/`, project `.grok/transcripts/`; do not glob across unrelated projects), a prior Grok session, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a child and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
