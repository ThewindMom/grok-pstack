...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript under the active workspace's `.grok/transcripts/` directory (the system prompt names the path). Don't glob across `~/.grok/projects/*/`; that reads unrelated private chats. Walk the log against what actually happened:
...
Before handing back, you must spawn a child on a different type from the one that did the work (`grok-pstack:pstack-high` if the parent was xhigh, `grok-pstack:pstack-xhigh` if the parent was high). Never `grok-4.5`. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. The child reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky. Spawn it from this parent session.
