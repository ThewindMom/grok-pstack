...
Before spawning investigators, list the MCP servers announced in this Grok session. Call `search_tool` for a tool's schema before the first `use_tool`. Do not guess parameter names. There is no Cursor `mcps/` directory.
