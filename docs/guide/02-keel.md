...
Grok nesting depth is 1. `/poteto-mode` running in this session owns every `spawn_subagent` call. A `poteto-agent` child cannot fan out. If a playbook needs parallel workers, the parent launches them.
...
Read [`pstack`](../../skills/poteto-mode/SKILL.md) itself for the full routing rules.
