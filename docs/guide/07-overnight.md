...
Before the skill hands back its summary, it spawns a reviewer on a different effort or persona to read the trail and the transcript, and the reply ends with an Attention section listing what deserves your scrutiny. Read that section first, then the log rows it points at. You're auditing decisions, not re-reading the whole night.
